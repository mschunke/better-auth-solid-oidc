...
export function createSolidProvider({
	...
	/**
	 * Client authentication for the token request, in the shape Better Auth's
	 * request builders expect. Solid clients identified by a Client Identifier
	 * Document are public clients, so `none` is the usual case.
	 */
	...
	/**
	 * Sends a token request with an RFC 9449 proof attached.
	 *
	 * Better Auth builds the body and the client authentication; the proof and
	 * the transport are owned here because a DPoP proof is bound to the exact
	 * request it accompanies, and because a provider may demand a
	 * server-supplied nonce that is only learned by being refused once.
	 *
	 * Redirects are refused rather than followed: the token endpoint carries the
	...
	const sendTokenRequest = async (
		...
		const attempt = async (nonce?: string) => {
			const headers = {
				...request.headers,
				dpop: await createSolidDpopProof({
					keyPair,
					method: "POST",
					url: tokenEndpoint,
					nonce,
			...
			return redirectRefusingFetch(tokenEndpoint, {
				method: "POST",
				body: request.body,
				headers,
			});
		...
		if (!response.ok) {
			// RFC 9449 §8: the authorization server can reject a first attempt
			// solely to hand out a nonce, and expects the same request back
			// carrying it. Retried once; a second refusal is a real failure.
			const nonce = response.headers.get("dpop-nonce");
			const body = await response.clone().text();
			if (nonce && body.includes("use_dpop_nonce")) {
		...
		if (!response.ok) {
			...
			ctx.logger.error(
				`Solid-OIDC provider "${providerId}": token endpoint responded ${response.status}. ${detail.slice(0, 500)}`,
			);
			throw APIError.from(
				"BAD_GATEWAY",
				SOLID_OIDC_ERROR_CODES.TOKEN_REQUEST_FAILED,
			);
		}
	...
	const provider: OAuthProvider = {
		...
		async validateAuthorizationCode(data) {
			...
			const tokens = await sendTokenRequest(
				request,
				keyPair,

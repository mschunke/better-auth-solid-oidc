...
import type { DpopSigningAlgorithm } from "better-auth/oauth2";
...
export function createSolidDpopKeyStore(ctx: AuthContext): SolidDpopKeyStore {
	...
	return {
		...
		async rebind({
			...
			if (!record) return;
			// Same guard as `load`: a row bound to another provider's token is
			// never rebound, so one provider cannot move another's key.
			if (record.providerId !== providerId) {
				ctx.logger.error(
					`Solid DPoP key for token hash ${previousHash} belongs to provider "${record.providerId}", not "${providerId}"`,
				);
				return;
			}

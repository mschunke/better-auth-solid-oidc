...
import { deriveDpopJkt, verifyDpopProof } from "better-auth/oauth2";
 ...
 * the shared helpers ask only for the members they use.
 */
...
async function completeAuthorization({
	...
	customFetchImpl,
	cookieSetter,
}: Pick<SignInHarness, "customFetchImpl" | "cookieSetter"> & {
	authUrl: string;
	headers: Headers;
}) {
...
describe("solidOidc dpop", () => {
	...
	/**
	 * RFC 9449 §8: an authorization server may refuse a first token request
	 * purely to hand out a nonce, and expects the same request back carrying it.
	 * The proof is re-signed rather than reused, because `jti` is single-use.
	 */
	it("retries the token request with a server-supplied DPoP nonce", async () => {
		...
		try {
			...
			expect(decodeJwt(proofs[1]!).nonce).toBe("server-issued-nonce");
			// A fresh jti per attempt: a proof is single-use.
			expect(decodeJwt(proofs[0]!).jti).not.toBe(decodeJwt(proofs[1]!).jti);
			expect(callbackURL).toBe("http://localhost:3000/dashboard");
		} finally {
			vi.unstubAllGlobals();
		}
	...
	it("gives up when the provider refuses a second time", async () => {
		const realFetch = globalThis.fetch;
		let tokenCalls = 0;
		vi.stubGlobal(
			"fetch",
		...
		try {
			...
			expect(callbackURL).toContain("error");
			expectLoggedError(/token endpoint responded 400/);
		} finally {
			vi.unstubAllGlobals();
		}
	});
